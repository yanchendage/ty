...
func (tw *TimeWheel) addTimer(tID uint32, t *Timer, forceNext bool) error {
	...
	fmt.Println("[addTimer]时间间隔:",delayInterval, "当前时间轮的单位",tw.interval)
	...
	if delayInterval >= tw.interval {
		...
		if tw.interval == SecondInterval {
			fmt.Println("[addTimer]添加到了秒轮上，当前index",tw.curIndex,"跨越的刻度数量",int(dn),"新的index:",(tw.curIndex+int(dn))%tw.scales)
		}
		if tw.interval == MinuteInterval {
			fmt.Println("[addTimer]添加到了分钟轮上，当前index",tw.curIndex,"跨越的刻度数量",int(dn),"新的index:",(tw.curIndex+int(dn))%tw.scales)
		}
...
func (tw *TimeWheel) run() {
	for {
		...
		if tw.interval == SecondInterval {
			fmt.Println("[run]秒轮前进，index:",tw.curIndex)
		}
		if tw.interval == MinuteInterval {
			fmt.Println("[run]分钟前进，index:",tw.curIndex)
		}
		...
		if len(curTimers) != 0 {
			if tw.interval == SecondInterval {
				fmt.Println("[run]秒轮取出挂载在当前刻度的全部定时器，index:",tw.curIndex)
			}
			if tw.interval == MinuteInterval {
				fmt.Println("[run]分钟轮取出挂载在当前刻度的全部定时器，index:",tw.curIndex)
			}
		}
		...
		if len(nextTimers) != 0 {
			if tw.interval == SecondInterval {
				fmt.Println("[run]秒轮取出下一个刻度挂载的全部定时器，index:",(tw.curIndex+1)%tw.scales)
			}
			if tw.interval == MinuteInterval {
				fmt.Println("[run]分钟取出下一个刻度挂载的全部定时器，index:",(tw.curIndex+1)%tw.scales)
			}
		}
