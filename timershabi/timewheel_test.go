...
func TestTimerWheel(t *testing.T) {
	...
	//给时间轮添加定时器
	timer6 := NewTimerAfter(NewDelayFunc(myFunc, []interface{}{5, 50}), 63*time.Second)
	_ = hourTw.AddTimer(6, timer6)
	fmt.Println("add timer 6 done!")
	//
	//return
	//timer7 := NewTimerAfter(NewDelayFunc(myFunc, []interface{}{7, 70}), 70*time.Second)
	//_ = hourTw.AddTimer(7, timer7)
	//fmt.Println("add timer 6 done!")
	//
	//timer8 := NewTimerAfter(NewDelayFunc(myFunc, []interface{}{8, 80}), 1*time.Minute)
	//_ = hourTw.AddTimer(8, timer8)
	//fmt.Println("add timer 8 done!")
